...
export async function getAllRewards() {
  try {
    ...
    console.log('Users with transactions:', usersWithTransactions);

    if (usersWithTransactions.length === 0) {
      return [];
    }

    // For each user, calculate their points from transactions
    const rewardsWithPoints = await Promise.all(
      usersWithTransactions.map(async (user) => {
        ...
        console.log(`Transactions for user ${user.userId}:`, transactions);
        ...
        console.log(`Total points for user ${user.userId}:`, totalPoints);

        // Get user's reward entry if it exists
        const [reward] = await db
          .select()
          .from(Rewards)
          .where(eq(Rewards.userId, user.userId))
          .execute();
    ...
    console.log('Rewards with calculated points:', rewardsWithPoints);

    // Sort by points in descending order
    const sortedRewards = rewardsWithPoints.sort((a, b) => b.points - a.points);
